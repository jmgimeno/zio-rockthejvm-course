...
object BusinessLogicSpec extends ZIOSpecDefault:
  ...
  val mockDatabase = ZIO.succeed(new Database[String, String] {
    ...
    def get(key: String): Task[String]              = ZIO.attempt(map(key))
    def put(key: String, value: String): Task[Unit] = ZIO.attempt(map += key -> value)
  })
  ...
  def spec = suite("a user survey application should...")(
    test("normalize user names") {
      val surveyPreliminaryLogic =
        ...
        yield normalized
      assertZIO(surveyPreliminaryLogic)(equalTo("DANIEL"))
    }
  ).provide(ZLayer.fromZIO(mockDatabase))
...
object BuiltInTestServicesSpec extends ZIOSpecDefault:
  def spec = suite("Checking built-in test services")(
    test("ZIO console application") {
      ...
      assertZIO(logicUnderTest)(
        hasSameElements(List("Please enter your name...", "Welcome, Daniel!"))
      )
    },
    test("ZIO clock") {
      val parallelEffect =
...
object AspectsSpec extends ZIOSpecDefault:
  ...
  def spec = suite("Testing Aspects")(
    ...
    /*
      - timeout(duration) - REAL execution time
      - eventually - retries until successful
      - nonFlaky(n) - repeats n times, stops at first failure
      - repeat(n) - the same
      - retries(n) - retries n times, stop at first success
      - debug - prints everything in the console
      - silent - prints nothing
      - diagnose(duration) - prints fiber dump if fails
