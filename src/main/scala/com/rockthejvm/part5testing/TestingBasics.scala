...
object MyFirstEffectTestSpec extends ZIOSpecDefault:
  def spec = test("First Effect Test") {
    ...
    assertZIO(person.saySomething)(
      Assertion.assertion("should be a greeting")(gr => gr == "Hi, I'm Daniel")
    )
    // doesn't work with assertTrue
    /*
      Assertion examples:
        - Assertion.assertion => tests any truth value = the most general value
        - Assertion.equalTo => tests for equality
        - Assertion.fails / failsCause => expects the effect to fail with the EXACT failure/case you specify
        - Assertion.dies => expects the effect to die with a Throwable, can run assertion on that Throwable
        - Assertion.isInterrupted => validates an interruption (can be a nested)
        - Assertion.isJustInterrupted => validates an interruption (immediate)
        - Specialized
...
object ASuiteSpec extends ZIOSpecDefault:
  def spec = suite("Full suite of tests")(
    ...
    test("a second test") {
      assertZIO(ZIO.succeed("Scala"))(
        Assertion.hasSizeString(Assertion.equalTo(5)) && Assertion.startsWithString("S")
      )
    },
    // sub-suites
